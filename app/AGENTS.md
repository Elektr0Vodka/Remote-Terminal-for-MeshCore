...
- `GET /packets/recent` — most recent raw packets (default 500, max 5000), oldest-first; seeds the packet feed on client mount/reconnect. Optional `after_ts` / `before_ts` (Unix seconds) filter by time window; used by the Mesh Visualizer historical packet mode
