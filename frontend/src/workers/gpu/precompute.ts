// ============================================================
// precompute.ts — Generate base point multiplication table for GPU
// Computes [0*B, 1*B, 2*B, ..., 15*B] using BigInt on CPU
// Serializes to Fe25519 limb format (16 × u32 per coordinate)
// Ported from meshcore-web-keygen/webgpu/precompute.js
// ============================================================

// Convert a BigInt field element to 16 × u16 limbs (matching tweetnacl gf format)
function bigintToLimbs16(n: bigint, p: bigint): Uint32Array {
  n = ((n % p) + p) % p;
...
type Point4 = [bigint, bigint, bigint, bigint]; // [X, Y, Z, T]

// Extended twisted Edwards point addition
function pointAdd(p1: Point4, p2: Point4): Point4 {
...
export function computeBaseTable(): Point4[] {
  ...
  for (let i = 1; i < 16; i++) {
    table.push(current.map((v) => ((v % P) + P) % P) as Point4);
...
export function serializeTableForGPU(table: Point4[]): Uint32Array {
...
// Combined helper: compute and serialize (cached after first call)
let _cachedTable: Uint32Array | null = null;
export function getBaseTableBuffer(): Uint32Array {
  if (!_cachedTable) {
    _cachedTable = serializeTableForGPU(computeBaseTable());
  }
  return _cachedTable;
}
