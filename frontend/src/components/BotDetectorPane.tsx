...
function ScoreBar({ value, color }: { value: number; color: string }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="w-8 shrink-0 text-right font-mono text-xs tabular-nums">{Math.round(value)}</span>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col h-full border-l border-border bg-card overflow-y-auto">
      <div className="flex items-center justify-between gap-2 shrink-0 border-b border-border px-4 py-3">
        ...
        <Button type="button" size="icon" variant="ghost" className="shrink-0 h-7 w-7" onClick={onClose}>
          <X className="h-4 w-4" />
        </Button>
      </div>
        ...
        <section>
          ...
          <div className="space-y-2">
            <div>
              ...
              <ScoreBar value={node.automation_score} color={automationColor(node.automation_score)} />
            </div>
            <div>
              <div className="flex justify-between text-xs mb-1">
                <span>Impact</span>
                  ...
                  return (
                    ...
                    >
                      <td className="px-4 py-2.5">
                        <div className="font-medium truncate max-w-[12rem]">{node.display_name}</div>
