...
async def run_migrations(conn: aiosqlite.Connection) -> int:
    ...
    if version < 57:
        ...
        applied += 1
    # Migration 58: Add statistics indexes for time-windowed scans (upstream migration 47)
    if version < 58:
        logger.info("Applying migration 58: add statistics indexes")
    ...
    if version < 61:
        logger.info("Applying migration 61: add discovery_blocked_types to app_settings")
        await _migrate_061_discovery_blocked_types(conn)
        await set_version(conn, 61)
        applied += 1

    if version < 62:
        logger.info("Applying migration 62: add foreign key cascade/set-null and clean orphans")
        await _migrate_062_foreign_key_cascade(conn)
...
async def _migrate_055_add_contact_advert_hash_mode(conn: aiosqlite.Connection) -> None:
    ...
    # Skip if contact_advert_paths.hash_mode doesn't exist yet (migration 54 may have failed).
...
async def _migrate_058_add_statistics_indexes(conn: aiosqlite.Connection) -> None:
...
async def _migrate_061_discovery_blocked_types(conn: aiosqlite.Connection) -> None:
...
async def _migrate_062_foreign_key_cascade(conn: aiosqlite.Connection) -> None:
