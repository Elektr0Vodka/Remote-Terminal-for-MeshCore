...
def _bool_col(row: Any, key: str, *, default: bool) -> bool:
    """Return a boolean column value, falling back to *default* on missing column or None."""
    try:
        val = row[key]
...
def _int_col(row: Any, key: str, *, default: int) -> int:
    """Return an integer column value, falling back to *default* on missing column or None."""
    try:
        val = row[key]
...
class AppSettingsRepository:
    ...
    async def _get_in_conn(conn: aiosqlite.Connection) -> AppSettings:
        ...
        return AppSettings(
            ...
            show_warning_ticker=_bool_col(row, "show_warning_ticker", default=True),
            show_mention_ticker=_bool_col(row, "show_mention_ticker", default=True),
            auto_delete_raw_enabled=_bool_col(row, "auto_delete_raw_enabled", default=False),
            auto_delete_raw_days=_int_col(row, "auto_delete_raw_days", default=14),
